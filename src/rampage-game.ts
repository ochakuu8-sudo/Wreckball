...
const BUILDING_COLORS: Partial<Record<C.BuildingSize, readonly [number, number, number]>> = {
  house: [0.72, 0.42, 0.30],
  townhouse: [0.78, 0.55, 0.36],
  garage: [0.46, 0.46, 0.44],
  shop: [0.42, 0.62, 0.82],
  convenience: [0.36, 0.78, 0.48],
  ramen: [0.84, 0.32, 0.22],
  izakaya: [0.74, 0.35, 0.24],
  apartment: [0.48, 0.58, 0.70],
  supermarket: [0.62, 0.82, 0.50],
  train_station: [0.86, 0.74, 0.45],
  office: [0.45, 0.70, 0.90],
  business_hotel: [0.78, 0.82, 0.90],
  apartment_tall: [0.54, 0.66, 0.80],
  warehouse: [0.48, 0.47, 0.42],
  factory_stack: [0.56, 0.48, 0.42],
  gas_station: [0.90, 0.78, 0.22],
  tower: [0.40, 0.72, 0.92],
  skyscraper: [0.34, 0.64, 0.88],
  department_store: [0.92, 0.75, 0.42],
  stadium: [0.70, 0.64, 0.54],
  castle: [0.96, 0.90, 0.92],
};
...
function roman(power: WreckPower): string {
  return ['', 'I', 'II', 'III', 'IV', 'V'][power] ?? 'V';
}

function buildingColor(kind: C.BuildingSize): readonly [number, number, number] {
  return BUILDING_COLORS[kind] ?? [0.62, 0.58, 0.50];
}
...
export class RampageGame {
  ...
  constructor(canvas: HTMLCanvasElement, _opts?: { screenshotMode?: boolean; screenshotChunkId?: number | null }) {
  ...
  private setupDom(): void {
    ...
    if (titleControls) titleControls.innerHTML = 'A / LEFT = L&nbsp;&nbsp;D / RIGHT = R<br>SPACE / CENTER TAP = BOTH';
    const tap = document.getElementById('title-tap');
    if (tap) tap.textContent = 'TAP / CLICK TO START';
    ...
    window.addEventListener('keydown', (e) => {
      if (this.state !== 'running' && !e.ctrlKey && !e.metaKey && !e.altKey) start(e);
    });
  }
  ...
  private updateBall(dt: number): void {
    const flL = this.flipper(false);
    const flR = this.flipper(true);
    ...
    if (this.ball.x < WORLD_LEFT + this.ball.r) {
      this.ball.x = WORLD_LEFT + this.ball.r;
      this.ball.vx = Math.abs(this.ball.vx) * 0.72;
    }
    if (this.ball.x > WORLD_RIGHT - this.ball.r) {
      this.ball.x = WORLD_RIGHT - this.ball.r;
      this.ball.vx = -Math.abs(this.ball.vx) * 0.72;
    }
    if (this.ball.y > this.cameraY + C.WORLD_MAX_Y - this.ball.r) {
      this.ball.y = this.cameraY + C.WORLD_MAX_Y - this.ball.r;
  ...
  private flipper(right: boolean): { ax: number; ay: number; bx: number; by: number; pressed: boolean; right: boolean } {
    ...
    const restDeg = right ? 205 : -25;
    const activeDeg = right ? 150 : 30;
    const deg = pressed ? activeDeg : restDeg;
    const a = deg * Math.PI / 180;
    return {
      ax: pivotX,
      ay: pivotY,
      bx: pivotX + Math.cos(a) * C.FLIPPER_W,
      by: pivotY + Math.sin(a) * C.FLIPPER_W,
      pressed,
      right,
    };
  }
  ...
  private resolveFlipper(f: ReturnType<RampageGame['flipper']>): void {
    ...
    const px = f.ax + dx * t;
    const py = f.ay + dy * t;
    const ox = this.ball.x - px;
    const oy = this.ball.y - py;
  ...
  private checkBuildingHits(): void {
    for (const b of this.buildings) {
      ...
      if (b.y + b.h < this.cameraY + C.WORLD_MIN_Y - 80) continue;
      if (b.y > this.cameraY + C.WORLD_MAX_Y + 160) continue;
  ...
  private bounceFromBuilding(b: Building, restitution: number): void {
    ...
    const dy = this.ball.y - (b.y + b.h / 2);
    if (Math.abs(dx) > Math.abs(dy)) {
      this.ball.vx = Math.sign(dx || 1) * Math.max(4, Math.abs(this.ball.vx)) * restitution;
    } else {
      this.ball.vy = Math.sign(dy || 1) * Math.max(4, Math.abs(this.ball.vy)) * restitution;
    }
  }
  ...
  private destroyBuilding(b: Building): void {
    ...
    for (let i = 0; i < 16 + b.tier * 8; i++) {
      this.particles.push({
        x: b.x + rand(-b.w / 2, b.w / 2),
        y: b.y + rand(0, b.h),
        vx: rand(-70, 70),
        vy: rand(-20, 90),
        life: rand(0.35, 0.75),
  ...
  private spawnHumansFromBuilding(b: Building): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const speed = rand(22, 68) + b.tier * 7;
      const away = Math.sign((b.x - this.ball.x) || rand(-1, 1));
      this.humans.push({
        x: b.x + Math.cos(a) * rand(4, 16),
        y: b.y + b.h * rand(0.2, 0.8),
        vx: Math.cos(a) * speed + away * 18,
  ...
  private updateBuildings(dt: number): void {
    for (const b of this.buildings) b.flash = Math.max(0, b.flash - dt);
  }
  ...
  private updateHumans(dt: number): void {
    for (const h of this.humans) {
      ...
      h.x += h.vx * dt;
      h.y += h.vy * dt;
      h.vx += (h.x >= this.ball.x ? 1 : -1) * 10 * dt;
      h.vy += 5 * dt;
      if (h.x < WORLD_LEFT + 6 || h.x > WORLD_RIGHT - 6) h.vx *= -0.65;
      if (h.life <= 0 || h.y < this.cameraY + C.WORLD_MIN_Y - 70) {
        h.active = false;
        continue;
      }
  ...
  private updateFailure(dt: number): void {
    if (this.momentum <= MOMENTUM_CRIT_THRESHOLD) this.lowMomentumTimer += dt;
  ...
  private addMomentum(amount: number): void {
    this.momentum = clamp(this.momentum + amount, 0, MOMENTUM_MAX);
  }
  ...
  private scrollSpeedForMomentum(): number {
    const t = clamp(this.momentum / MOMENTUM_MAX, 0, 1);
    return 22 + (96 - 22) * t;
  }
  ...
  private spawnDistrict(baseY: number): void {
    ...
    for (let i = 0; i < district.density; i++) {
      const kind = pick(district.buildings);
      const def = C.BUILDING_DEFS[kind];
      const x = pick(lanes) + rand(-8, 8);
      const y = baseY + rand(18, CHUNK_H - 26);
      this.buildings.push({
        id: this.buildingId++,
        kind,
        x,
        ...
        maxHp: def.hp,
        score: def.score,
        humans: randInt(def.humanMin, def.humanMax) + district.humans,
        tier: requiredWreckTier(kind),
        active: true,
        flash: 0,
        color: buildingColor(kind),
      });
    }
    const ambientHumans = district.kind === 'residential' ? 5 : Math.floor(district.humans * 0.35);
    for (let i = 0; i < ambientHumans; i++) {
      this.humans.push({
        x: rand(WORLD_LEFT + 18, WORLD_RIGHT - 18),
        y: baseY + rand(12, CHUNK_H - 10),
        vx: rand(-22, 22),
        vy: rand(-8, 18),
        life: rand(5, 9),
        value: 0.65,
  ...
  private spawnPopup(x: number, y: number, text: string): void {
    ...
    popup.className = 'world-popup score';
    popup.textContent = text;
    popup.style.left = `${x + FIELD_W / 2}px`;
    popup.style.top = `${FIELD_H / 2 - (y - this.cameraY)}px`;
  ...
  private updateHud(): void {
    ...
    if (this.lifeWrapEl) {
      this.lifeWrapEl.classList.toggle('low', this.momentum <= MOMENTUM_LOW_THRESHOLD && this.momentum > MOMENTUM_CRIT_THRESHOLD);
      this.lifeWrapEl.classList.toggle('crit', this.momentum <= MOMENTUM_CRIT_THRESHOLD);
    }
    const power = this.getWreckPower();
    ...
    if (this.bestEl) {
      if (this.bestScore > 0) {
        this.bestEl.textContent = `BEST ${this.bestScore.toLocaleString()}`;
  ...
  private render(): void {
    const power = this.getWreckPower();
    this.renderer.updateProjection(this.cameraY);
    const t = clamp(this.momentum / 100, 0, 1);
    this.renderer.drawBackground(0.10 + t * 0.10, 0.12 + t * 0.05, 0.18, 0.05, 0.04, 0.05);
    let n = 0;
    n = this.drawRoads(n);
    n = this.drawBuildings(n, power);
    n = this.drawHumans(n);
    n = this.drawParticles(n);
    n = this.drawFlippers(n);
    n = this.drawBall(n);
  ...
  private drawBuildings(n: number, power: WreckPower): number {
    for (const b of this.buildings) {
      if (!b.active) continue;
      if (b.y + b.h < this.cameraY + C.WORLD_MIN_Y - 80 || b.y > this.cameraY + C.WORLD_MAX_Y + 120) continue;
      const col = b.flash > 0 ? [1, 1, 1] as const : b.color;
      const canBreak = power >= b.tier;
      ...
      if (b.maxHp > 1) {
        for (let i = 0; i < Math.max(0, b.hp); i++) writeInst(SHARED_BUF, n++, b.x - b.w / 2 + 5 + i * 5, b.y + b.h + 5, 3, 3, 1, 0.25, 0.15, 1);
      }
    }
    return n;
  }

  private drawHumans(n: number): number {
    for (const h of this.humans) {
      if (!h.active) continue;
      if (h.y < this.cameraY + C.WORLD_MIN_Y - 20 || h.y > this.cameraY + C.WORLD_MAX_Y + 20) continue;
      writeInst(SHARED_BUF, n++, h.x, h.y, C.HUMAN_W, C.HUMAN_H, 1.0, 0.82, 0.52, 1);
    }
    return n;
  }

  private drawParticles(n: number): number {
    for (const p of this.particles) {
      if (p.y < this.cameraY + C.WORLD_MIN_Y - 20 || p.y > this.cameraY + C.WORLD_MAX_Y + 20) continue;
      writeInst(SHARED_BUF, n++, p.x, p.y, p.size, p.size, p.color[0], p.color[1], p.color[2], clamp(p.life * 2, 0, 1));
    }
    return n;
  }

  private drawFlippers(n: number): number {
    const l = this.flipper(false);
    const r = this.flipper(true);
    const drawOne = (f: ReturnType<RampageGame['flipper']>) => {
      const cx = (f.ax + f.bx) / 2;
      const cy = (f.ay + f.by) / 2;
      const angle = Math.atan2(f.by - f.ay, f.bx - f.ax);
      writeInst(SHARED_BUF, n++, cx, cy, C.FLIPPER_W, 8, 1, 0.82, 0.12, 1, angle);
    };
    drawOne(l);
    drawOne(r);
    return n;
  ...
  private drawBall(n: number): number {
    const hot = this.overdriveTimer > 0;
    writeInst(SHARED_BUF, n++, this.ball.x, this.ball.y, this.ball.r * 2, this.ball.r * 2, hot ? 1 : 1, hot ? 0.88 : 0.22, hot ? 0.18 : 0.10, 1, 0, 1);
    writeInst(SHARED_BUF, n++, this.ball.x - 5, this.ball.y + 5, 6, 6, 1, 0.75, 0.55, 1, 0, 1);
    return n;
  }
}
